...
def main(args):
    ...
    for fold in range(1, 6):
        ...
        df_train = pd.read_csv(f'data/{dataset}/5 fold/train{fold}.csv')# Reading training data.
        df_test = pd.read_csv(f'data/{dataset}/5 fold/test{fold}.csv') # Reading test data.
        ...
        for epoch in range(args.epochs):
            ...
            ret = [mse(G, P), concordance_index(G, P),rm2(G, P)]
            if ret[0] < best_mse:
        ...
        results.append([ret[0],ret[1],ret[2]])
    valid_results = np.array(results)
    print('fold_1: ', results[0])
    print('fold_2: ', results[1])
    print('fold_3: ', results[2])
    print('fold_4: ', results[3])
    print('fold_5: ', results[4])
    valid_results = [np.mean(valid_results, axis=0), np.std(valid_results, axis=0)]
    print("5-fold cross validation finished. " "mse:{:.3f}±{:.4f} | CI:{:.3f}±{:.4f} | rm2:{:.3f}±{:.4f},".format(valid_results[0][0], valid_results[1][0], valid_results[0][1], valid_results[1][1],valid_results[0][2], valid_results[1][2]))
