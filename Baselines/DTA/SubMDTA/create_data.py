...
def fun(prots,ngram):
    ...
    XT = split_sequence(prots, ngram=ngram)
    ...
    if pro_len > seq_max:
        ...
        pro = XT + [0] * (seq_max - pro_len)
        mask = [1] * pro_len + [0] * (seq_max - pro_len)
    pro = np.asarray(pro)


    return torch.LongTensor([pro])
...
def construct_graph(args):
    ...
    if args.dataset == 'PDBbind':
        ...
        with open(f'{fpath }/protein_sequences.txt', 'r') as f:
            for line in f:
                ...
                if len(parts) == 2:
                    protein_id = parts[0]
                    sequence = parts[1]
                seq_dict[protein_id] = [fun(sequence,2),fun(sequence,3),fun(sequence,4)]
        # seq_feats = {}
        ...
        seq_feats = {}
        for pro_id, seq in seq_dict.items():
            seq_feats[seq] = [fun(seq,2),fun(seq,3),fun(seq,4)]
        print('protein graph is constructed successfully!')
