...
def mol2_to_smiles_with_rdkit(mol2_path):
    """使用RDKit直接从mol2转换为SMILES"""
    try:
        # 尝试各种读取mol2的参数组合
        mol = Chem.MolFromMol2File(mol2_path)  # 默认尝试
        if not mol:
            mol = Chem.MolFromMol2File(mol2_path, sanitize=True)  # 开启化学验证
        if not mol:
            mol = Chem.MolFromMol2File(mol2_path, removeHs=False)  # 保留氢原子
        if not mol:
            mol = Chem.MolFromMol2File(mol2_path, cleanupSubstructures=True)  # 清理亚结构

        if mol:
            # 生成规范化的SMILES
            return Chem.MolToSmiles(mol, isomericSmiles=True, canonical=True)
    except Exception as e:
        print(f"RDKit转换失败: {str(e)}")
    return None


# def mol2_to_smiles_with_obabel(mol2_path):
#     """使用OpenBabel作为备选转换mol2为SMILES"""
#     try:
#         # 直接通过OpenBabel生成SMILES
#         mol = next(pybel.readfile('mol2', mol2_path))
#         return mol.write(format='smi').split()[0].strip()  # 提取第一部分为SMILES
#     except Exception as e:
#         print(f"OpenBabel转换失败: {str(e)}")
#     return None


def sdf_to_smiles_with_rdkit(sdf_path):
    """使用RDKit从SDF转换为SMILES"""
    try:
...
def construct_graph(args):
    ...
    for id in progress_bar:
        mol2 = os.path.join(index_path, id, f"{id}_ligand.mol2")
        sdf = os.path.join(index_path, id, f"{id}_ligand.sdf")
        smiles = None

        # 1. 优先尝试SDF文件 (RDKit通常更好地支持SDF)
        if os.path.exists(sdf):
            smiles = sdf_to_smiles_with_rdkit(sdf)

        # 2. 如果SDF失败，尝试MOL2文件
        if not smiles and os.path.exists(mol2):
            # 先用RDKit尝试
            smiles = mol2_to_smiles_with_rdkit(mol2)

            # 如果RDKit失败，使用OpenBabel

        if smiles:
            drug_dict[id] = smiles
            success_count += 1
            # 更新进度条状态
            progress_bar.set_postfix_str(f"成功: {success_count}/{total_count} ID: {id[:6]}")
        else:
            failed_cases.append(id)
            print(f"\n警告: 无法转换配体 {id}")
    # 输出统计信息
    print(f"\n转换完成! 成功: {success_count}/{total_count} ({success_count / total_count:.2%})")
    ...
    df_train = pd.read_csv(f'data/{args.dataset}/train_o.csv')  # Reading training data.
    df_test = pd.read_csv(f'data/{args.dataset}/test_o.csv')  # Reading test data.

    def get_smiles(id):
        return drug_dict.get(id, None)  # 使用get避免KeyError

    def get_sequence(id):
        return seq_dict.get(id, None)

    df_train['smiles'] = df_train['id'].apply(get_smiles)
    df_train['sequence'] = df_train['id'].apply(get_sequence)

    # 为测试集添加新列
    df_test['smiles'] = df_test['id'].apply(get_smiles)
    df_test['sequence'] = df_test['id'].apply(get_sequence)

    # 创建新的数据集（只包含所需三列）
    new_train = df_train[['smiles', 'sequence', 'affinity']].rename(columns={
        'smiles': 'compound_iso_smiles',
        'sequence': 'target_sequence',
        'affinity': 'affinity'
    })

    new_test = df_test[['smiles', 'sequence', 'affinity']].rename(columns={
        'smiles': 'compound_iso_smiles',
        'sequence': 'target_sequence',
        'affinity': 'affinity'
    })
    ...
    # 可选：保存新数据集
    new_train.to_csv(f'data/{args.dataset}/train.csv', index=False)
    new_test.to_csv(f'data/{args.dataset}/test.csv', index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default= 'PDBbind', help='dataset name',choices=['davis','kiba','Human','PDBbind'])
